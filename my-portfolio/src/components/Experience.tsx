    ...
    return items.map((item: any, index: number) => <Timeline.Item data-aos="fade-up" data-aos-duration="800"
        ...
        <div className="border shadow-[0_0_10px_0_#64FFDA50] hover:-translate-y-2 transition transform duration-300 ease-in-out flex flex-col gap-2 border-primaryColor p-4 rounded-2xl sm-mx:p-2">
            <div className="flex gap-2 items-center">
                <div className="w-24 h-12 md-mx:w-20 md-mx:h-10 rounded-lg overflow-hidden bg-[#d9d9d9] flex items-center justify-center border border-slate-600/40">
                    <img
                        className="w-full h-full object-contain"
                        src={`${process.env.PUBLIC_URL}/${encodeURIComponent(item.company)}.png`}
                        alt={item.company}
                        onError={(e) => {
                            const target = e.currentTarget;
                            if (!target.dataset.fallbackApplied) {
                                target.dataset.fallbackApplied = "true";
                                target.src = `${process.env.PUBLIC_URL}/${encodeURIComponent(item.company)}.svg`;
                            }
                        }}
                    />
                </div>
...
const Experience = () => {
    ...
    return <div className="px-16 mx-20 md-mx:px-6 sm-mx:px-2 lg-mx:mx-0 my-10 mb-28 font-mono" id="Experience">
        <h1 className="text-5xl sm-mx:text-4xl xs-mx:text-3xl tracking-tight mb-10 font-extrabold text-center text-white"><span className="text-primaryColor">04.&nbsp;</span>Experience</h1>
